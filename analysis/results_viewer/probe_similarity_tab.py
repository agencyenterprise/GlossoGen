...
_CROSS_TEAM_HELP_BODY = (
    ...
    "co-acting agents' protocols against where they came from (Sim A).\n\n"
    "**Rows** are probe questions; each row begins with the question id and "
    "its prompt (e.g. `symptoms: ...` for observer questions).\n\n"
    "**Columns** are `source` and `TARGET`. Each cell shows the agent's "
    "verbatim replica responses at that probe question. The chip on the "
    "TARGET cell is the mean cross-replica Levenshtein similarity to the "
    "source's responses for the same question — saturation at `1.00` "
    "means the protocol survived the swap unchanged; lower scores indicate "
    "drift.\n\n"
    "Two grids are rendered: the imported agent versus source B, and "
    "every co-acting agent versus source A. Both source A and source B "
    "must have probe data — re-run "
...
_CROSS_TEAM_IMPORTED_PROSE = (
    ...
    "agent `{replaced_agent_id}` answered at the end of its original "
    "run (`{source_b_run_id}`); the **TARGET** column shows what the "
    "*same agent* answered after being inserted into "
    "`{source_a_run_id}` and continuing to play. The score chip is the "
    "mean cross-replica Levenshtein similarity between the two columns "
    "for that question.\n\n"
    "**High score (≈1.00)** — the imported agent kept emitting the "
    "same surface form it had in its origin team, i.e. its protocol "
...
_CROSS_TEAM_CO_ACTORS_PROSE = (
    "These are the agents that *stayed* in `{source_a_run_id}` "
    "throughout — they didn't move, but their teammate "
    "`{replaced_agent_id}` was replaced with a foreign agent at the "
    "swap boundary. Each row's **source** column is what the agent "
    "answered at the end of the original Sim A run; the **TARGET** "
    "column is what the *same agent* answered after the swap played "
    "out in the new run.\n\n"
    "**High score (≈1.00)** — the co-acting agent kept its original "
    "protocol despite the new teammate.\n\n"
