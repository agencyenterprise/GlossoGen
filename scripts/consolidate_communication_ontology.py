...
``<runs_dir>/<scenario_name>/_ontology/`` and is the input to pass 3
(``communication_feature_presence``).

Scenario-agnostic: pass ``--scenario-name`` to anchor the output path;
each scenario's open-coding labels consolidate into a distinct
...
from glossogen.llm.provider import LLMMessage
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "sidecar and writes one CommunicationOntology JSON file under "
            "<runs_dir>/<scenario_name>/_ontology/<version>.json."
        )
    )
    parser.add_argument(
        "--scenario-name",
        ...
        required=True,
        help=(
            "Scenario these runs belong to (e.g. 'veyru', "
            "'container_yard_stacking'). Used to namespace the output path."
        ),
    )
    parser.add_argument(
        "--run-id",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Version stem for the ontology JSON. Output is written to "
            "<runs_dir>/<scenario_name>/_ontology/<version>.json."
        ),
    )
    parser.add_argument(
...
def _load_sidecars(runs_dir: Path, run_ids: list[str], min_runs: int) -> LoadedSidecars:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            + ", ".join(missing)
            + ". Run `glossogen evaluate ... --metrics communication_open_coding` first."
        )
...
async def _main() -> int:
    ...
    if not runs_dir.exists():
        ...
        return 1
    output_path = (
        ontology_dir_for_scenario(runs_dir=runs_dir, scenario_name=args.scenario_name)
        / f"{args.version}.json"
    )
