...
- `src/glossogen/db/` — Postgres data layer (raw SQL via psycopg3 async; alembic for migrations)
  ...
  - `mcp/browser.py` — MCP server mounted at `/mcp` for programmatic run browsing and launching (Codex, Cursor)
...
The backend exposes an MCP (Model Context Protocol) server at `/mcp` for programmatic access to simulation data from LLM clients like Codex or Cursor. The MCP server is mounted inside the existing FastAPI app and uses OAuth 2.0 for authentication. Requires `OAUTH_ISSUER_URL` to be set.
...
```bash
Codex mcp add-json glossogen-runs '{"type":"http","url":"<API_URL>/mcp"}'
```
...
I (Codex) once destroyed eval-derived labels on 40 runs by writing this:
...
VIRTUAL_ENV= uv run --no-sync python -m glossogen run veyru \
  --model Codex-sonnet-4-6 --provider anthropic --runs-dir ./runs \
  --config src/glossogen/scenarios/veyru/knobs_default.json \
  > ./runs/veyru_stdout.log 2>&1 &
...
glossogen replace-agent veyru \
  ...
  --model Codex-sonnet-4-6 --provider anthropic \
  --runs-dir ./runs \
  [--rounds-after-swap N] \
...
```bash
glossogen run veyru --model Codex-sonnet-4-6 --provider anthropic --runs-dir ./runs \
...
```json
{
  ...
  "model_overrides": {
    ...
    "field_observer": {"model": "Codex-opus-4-6", "provider": "anthropic"}
  },
  "round_count": 12
}
```
...
- **Run a separate queue per model in parallel** so a paused `gpt-5.4` queue (waiting for an OpenAI slot) never holds back the `Codex-sonnet-4-6` queue. Strict-sequential single-queue orchestrators are a bug: with mixed-model specs, the queue blocks on the current spec's model and idles every slot on the other provider until the current spec launches. Always use per-provider parallel queues — typically two background subshells joined by `wait`.
...
process_queue_sonnet() {
  for spec in "${SONNET_SPECS[@]}"; do
    read -r source knobs <<< "$spec"
    while [ "$(count_running_for_model Codex-sonnet-4-6)" -ge 6 ]; do sleep 30; done
    launch_one Codex-sonnet-4-6 anthropic "$source" "$knobs"
  done
