#!/bin/bash
# Multi-swap baseline (sonnet) — postmortem-always-on variant — 2026-05-22
# Identical to the sibling cohort except `set_postmortem(at_round=11, off)` is
# omitted, so postmortem stays enabled throughout all 4 phases.
set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$SCRIPT_DIR/../.."

LOG=/tmp/multi_swap_baseline_postmortem_on_sonnet.log
CONFIG="experiments/2026-05-22_multi_swap_baseline_postmortem_on_sonnet/knobs.json"
LABELS_JSON='["multi_swap_baseline_postmortem_on", "budget=450", "phases=A10-B10-C10-D10", "history=10"]'
START_REP=1
TOTAL_RUNS=10
...
echo "=== Multi-swap postmortem-on launcher started $(date) ===" >> "$LOG"
for rep in $(seq "$START_REP" "$TOTAL_RUNS"); do
  while [ "$(count_running_sonnet)" -ge 6 ]; do
    sleep 30
  done

  rep_log="/tmp/multi_swap_baseline_pm_rep${rep}.log"
  : > "$rep_log"
...
echo "=== Multi-swap postmortem-on launcher complete $(date) ===" >> "$LOG"
