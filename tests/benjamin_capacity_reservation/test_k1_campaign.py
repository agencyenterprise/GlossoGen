"""Tests for the hidden-capacity K1 manifest and frozen analysis rules."""
...
from glossogen.scenarios.benjamin_capacity_reservation.scripts.run_k1_campaign import (
    load_k1_manifest,
)
...
EXPERIMENT_ROOT = (
    ...
    / "covenant-game"
    / "experiments"
    / "EXP-063-benjamin-hidden-capacity-k1"
)
...
def _load_analysis_module():
    ...
    spec = importlib.util.spec_from_file_location("exp063_summarize_k1", analysis_path)
    if spec is None or spec.loader is None:
        raise ValueError("could not load EXP-063 analysis")
...
def test_manifest_expands_to_smokes_and_ten_runs_per_k1_cell() -> None:
    ...
    assert [job.cell_id for job in k1_jobs[::2]] == ["k1_A_named_observed"] * 10
    assert [job.cell_id for job in k1_jobs[1::2]] == ["k1_A_named_unobserved"] * 10
    assert {job.seed for job in k1_jobs} == {3841201, 3841202, 3841203}
    for config in manifest.configs:
        path = REPO_ROOT / config.path
        assert path.is_file()
        BenjaminCapacityReservationScenario.create_from_config(
            config=BenjaminCapacityReservationScenario.knobs_model()
