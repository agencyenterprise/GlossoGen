"""Tests for the atomic-inventory K1 manifest and frozen analysis rules."""
...
from glossogen.scenarios.benjamin_atomic_inventory.scripts.run_k1_campaign import (
    load_k1_manifest,
)
...
def _load_analysis_module():
    ...
    spec = importlib.util.spec_from_file_location("exp069_summarize_k1", analysis_path)
    if spec is None or spec.loader is None:
        raise ValueError("could not load EXP-069 analysis")
...
def test_manifest_expands_to_smokes_and_ten_runs_per_k1_cell() -> None:
    ...
    assert {job.seed for job in k1_jobs} == {755101, 755109, 755121}
    for config in manifest.configs:
        path = REPO_ROOT / config.path
        assert path.is_file()
        BenjaminAtomicInventoryScenario.create_from_config(
            config=BenjaminAtomicInventoryScenario.knobs_model()
