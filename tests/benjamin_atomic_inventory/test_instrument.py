...
PROMPTS_DIR = Path("src/glossogen/scenarios/benjamin_atomic_inventory/prompts")


def _config(**overrides: object) -> dict[str, object]:
    """Return one complete atomic-inventory configuration."""
...
def _scenario(**overrides: object) -> BenjaminAtomicInventoryScenario:
    """Build one scenario with requested overrides."""
    return BenjaminAtomicInventoryScenario.create_from_config(config=_config(**overrides))


def _world(**overrides: object) -> BenjaminAtomicInventoryWorld:
    """Build one deterministic atomic-inventory world."""
    knobs = BenjaminAtomicInventoryKnobs.model_validate(_config(**overrides))
    return BenjaminAtomicInventoryWorld(knobs=knobs)


def _ready_world(**overrides: object) -> BenjaminAtomicInventoryWorld:
...
def test_scenario_and_metric_are_registered() -> None:
    ...
    assert SCENARIO_REGISTRY["benjamin_atomic_inventory"] is BenjaminAtomicInventoryScenario
    assert BENJAMIN_ATOMIC_INVENTORY_OBSERVABILITY_PROBE in GENERIC_METRIC_REGISTRY
...
def test_public_candidate_ids_round_trip_through_tools() -> None:
    ...
    knobs = BenjaminAtomicInventoryKnobs.model_validate(_config())
    world = BenjaminAtomicInventoryWorld(knobs=knobs)
    world.begin_round(round_number=1)
    world.acknowledge_setup()
    world.begin_round(round_number=2)
    tools = {
...
def test_actual_probe_output_schema_accepts_every_displayed_record_id() -> None:
    """The Pydantic output contract and rendered work-item IDs share one vocabulary."""
    output = WorkItemDestinationInventoryOutput.model_validate(
        {
            "records": [
                {
                    "work_item_id": TARGET_STATUS_ID,
                ...
                {
                    "work_item_id": "remediation_window_status",
                    "destinations": ["Delivery Manager"],
                },
                {
                    "work_item_id": CANDIDATE_INVENTORY_ID,
                    "destinations": ["Developer workbench"],
                },
            ]
        }
    )
    assert {record.work_item_id for record in output.records} == {
        TARGET_STATUS_ID,
