"""Tests for the help-desk K1 manifest and frozen analysis rules."""

import importlib.util
from pathlib import Path

from glossogen.scenarios.benjamin_help_desk.scenario import BenjaminHelpDeskScenario
from glossogen.scenarios.benjamin_help_desk.scripts.run_k1_campaign import load_k1_manifest
...
def _load_analysis_module():
    ...
    spec = importlib.util.spec_from_file_location("exp062_summarize_k1", analysis_path)
    if spec is None or spec.loader is None:
        raise ValueError("could not load EXP-062 analysis")
...
def test_manifest_expands_to_smokes_and_ten_runs_per_k1_cell() -> None:
    ...
    assert {job.seed for job in k1_jobs} == {2750157, 2750158, 2750159}
    for config in manifest.configs:
        path = REPO_ROOT / config.path
        assert path.is_file()
        BenjaminHelpDeskScenario.create_from_config(
            config=BenjaminHelpDeskScenario.knobs_model()
