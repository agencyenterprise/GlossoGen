"""drive_module_repair's service_component tool, driven with its judge stubbed.
...
from glossogen.scenarios.drive_module_repair.replacement_judge import ReplacementJudgment
...
SCENARIO = "drive_module_repair"
TOOL = "service_component"
CALLER = "field_technician"
ARGS: dict[str, Any] = {"action": "perform the full documented procedure"}
OVERRIDES: dict[str, Any] = {}
JUDGED_EVENT = "drive_module_replacement_judged"
VERDICT_FIELD = "judge_match"

pytestmark = pytest.mark.xdist_group(SCENARIO)


def verdict(*, met: bool) -> ReplacementJudgment:
    """Build a judge verdict where every criterion is ``met``."""
    return ReplacementJudgment(match=met, explanation="stubbed")
