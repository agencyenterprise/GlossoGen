...
3. If vulture reports new false positives, regenerate the whitelist with `make vulture-whitelist`. It scans the same paths `make lint-server` checks, so it cannot drop the entries covering whatever a narrower run left out, and it writes the names as a tuple. Do not use `vulture --make-whitelist` directly: it emits one bare name per line, and a bare name is a statement with no effect, which the CodeQL `code-quality` check reports for every line of the file.
