...
├── communication_open_coding.json               # (veyru only, when communication_open_coding metric is run) free-form open-coding labels for this run
├── communication_feature_presence.json          # (veyru only, when communication_feature_presence metric is run) per-category confidence vector against a consolidated ontology
...
- **veyru**: generic + the following veyru-specific metrics:
  ...
  - `communication_open_coding` — pass 1 of the open-coding → ontology → relabel pipeline. One LLM call per run feeds the judge every link-channel message plus the per-round motif → treatment ground truth, and asks for free-form short labels naming communication-pattern features (multi-label per run, no pre-specified vocabulary). Writes `communication_open_coding.json` to the run dir with each label's evidence round and quote. `score` = number of free-form labels. Followed by `src/schmidt/scenarios/veyru/scripts/consolidate_communication_ontology.py` (one LLM call across N runs, writes a versioned ontology under `analysis/communication_ontology/<version>.json`) and then `communication_feature_presence` for relabel.
  - `communication_feature_presence` — pass 3 of the same pipeline. Requires `--ontology-path PATH` on the eval CLI pointing at a consolidated ontology JSON. One LLM call per run re-reads the link messages + ground-truth against the ontology's categories and emits a 0–1 confidence per category. Writes `communication_feature_presence.json` (full feature-presence vector + ontology provenance). `score` = number of categories scoring ≥0.5. Pass-1 and pass-3 use the same `build_link_rounds()` transcript view so confidences are commensurable with the open-coding labels.
