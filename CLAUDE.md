...
3. If vulture reports new false positives, regenerate the whitelist with `make vulture-whitelist`. It scans the same paths `make lint-server` checks, or the regenerated file drops the entries covering the ones it left out. The whitelist is `vulture_whitelist.txt`, not a `.py` file: vulture parses whatever path it is given, and the extension is what keeps the CodeQL `code-quality` check from reading every bare name in it as a statement with no effect.
