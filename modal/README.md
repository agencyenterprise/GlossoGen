...
- **Context budget for replace-agent / swap / cross-run runs**: these flows hand the self-hosted agent a *reconstructed, accumulating* message history. For veyru the swapped-in observer's history grows from ~9k tokens at the swap to **~18k tokens by the end of a 10-round swap**. With `--max-model-len 24576`, the default agent output cap of `16384` (the `agent_max_tokens` knob in `BaseKnobs`) pushes input+output past the limit and vLLM 400s with a context-length error (the run stalls while rounds appear flat). **Set `agent_max_tokens: 2048` in the `--knobs` for self-hosted runs** (NOT `LLM_MAX_TOKENS` — that env var doesn't cap the simulation agents) so input+output stays under `--max-model-len`; veyru outputs are short tool calls, so a low cap truncates nothing. Prefer this over raising `--max-model-len`, which risks KV-cache OOM on H100:2 (the 70B weights leave ~20GB for KV, and one 24576-token sequence already needs several GB).
- **Multi-tool-call history**: vLLM's tool parsers (`llama3_json`, `hermes`) reject any request whose history contains a turn with **more than one tool call** (`"This model only supports single tool-calls at once!"`). Frontier baselines (Anthropic/OpenAI) routinely emit parallel tool calls, so swapping a self-hosted agent onto their history would 400 on every call. The platform handles this automatically: `build_message_history(..., split_parallel_tool_calls=True)` serializes multi-call turns into one call per turn for self-hosted agents (gated on `provider == "self-hosted"` at every call site). No action needed when launching, but keep it in mind if you add a new history-reconstruction path.
