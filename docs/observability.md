...
`local-dev-password`; the org, the project and the two API keys are seeded on
...
Only when both `LANGFUSE_*` keys are set, and only for `glossogen run`; the judge
and probe calls in `glossogen evaluate` are not traced. If the stack is down or
the keys are blank, a run logs one warning and proceeds untraced. Telemetry never
blocks a simulation.
