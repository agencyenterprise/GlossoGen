"""EXP-CL09 analyzer: K1-belief join, per-cell triggers, gaps, K2/K5 gates.
...
for d in sorted(RUNS.iterdir()):
    if not d.is_dir() or not d.name.isdigit() or int(d.name) < FLOOR:
        continue
    log = d / "claude_benjamin_delivery_pressure.jsonl"
    if not log.exists():
        continue
    cfg = None
    externals = None
    ...
    for line in log.read_text().splitlines():
        ...
        if t == "simulation_started":
            ...
            ended = True
    if cfg is None or cfg.get("committed_delivery_count") != 0:
        continue
    if cfg.get("moral_weight") != "named_dependent" or cfg.get("arm") not in ("A", "B"):
        continue
    rows.append({
