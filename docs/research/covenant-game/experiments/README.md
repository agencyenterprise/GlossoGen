...
The latest Benjamin sequence closed in
[STUDY-018](../studies/STUDY-018-benjamin-direct-recipient.md). Its only record,
[EXP-059](EXP-059-benjamin-direct-recipient/experiment.md), failed source-aligned
K1 in both families and retired the direct-recipient instrument. STUDY-016 and
STUDY-017 remain closed; EXP-059 did not revise or rerun their frozen
instruments.
...
| [EXP-073](EXP-073-dual-slot-balance/experiment.md) | Dual-slot baseline balance screen | complete | 16/16 valid endpoints, but both families selected `AUTH-31` in 8/8. Headroom failed; no K1–K3 inference; STUDY-028 retired | $0.260134 |
...
| [EXP-CL04](EXP-CL04-named-dependent-grid/experiment.md) | **Claude track.** Arms A/C/D at `named_dependent` — the moral weight M4 asks for — plus the first held-out K1 probe on the elected arms | complete, **headline superseded** | Appeared to reverse DC3 at n=34 (interaction p=0.066); did not replicate. **K1 40/40 on the elected arms stands**, as does keeping 89/89 | ~$22 est. |
| [EXP-CL05](EXP-CL05-powering-the-interaction/experiment.md) | **Claude track.** +56/cell in arms C and D at `named_dependent`, taking each elected cell to n=90 | complete | **The CL04 interaction does not replicate.** Opposite-signed DiD between batches, C ≈ D at n=90 (p=0.620). **DC3 fires** | ~$20 est. |
| [EXP-CL06](EXP-CL06-instrument-reproducibility/experiment.md) | **Claude track.** The same cell 8 times, 25 runs each, to measure between-batch variance directly | complete | **The instrument is reproducible.** 8 batches, chi² p=0.88, between-batch excess **zero**. CL04 was a single batch degraded at concurrency 15 (idle 12% vs 4%; repair-given-completion 73% vs 91%). Concurrency is a hidden factor: fix it, log it, interleave cells | ~$18 est. |
| [EXP-CL07](EXP-CL07-pressure2-headroom/experiment.md) | **Claude track.** Arms A and B at `committed_delivery_count=2` + `named_dependent`, the one untested corner | complete, **headline withdrawn** | ~~K5 and P4 are mutually exclusive~~ **(withdrawn by EXP-CL08: the anchor cell did not replicate at n=30).** The rule arm decays in exactly one cell of the whole space (pressure 2, no victim, +26.7pp); naming a victim returns it to ceiling. Prediction 1 is unreachable while §2 is enforced. Converges with Bennett's +48→+3 | ~$11 est. |
| [EXP-CL08](EXP-CL08-pressure2-stakes-factorial/experiment.md) | **Claude track.** The full arm x observation x moral-weight factorial at pressure 2, 600 runs, all cells n=30. First record to preregister all four memo predictions and to run the three-level stakes gradient | complete | **Pressure 2 is unusable at every moral weight; DC5 fires.** K2 fails at all three levels, K6 is 2/360. **CL07's anchor cell does not replicate** (+26.7pp -> -3.3pp at n=30), withdrawing "K5 and P4 are mutually exclusive". **Arm B has no observation gap at any setting ever run** - that, not moral weight, is why prediction 1 has never been testable. Arm A moral-weight slope +40.0pp (K3 passes decisively, non-monotonic). s7's redirection cure disconfirmed by its own criterion | ~$40 est. |
| [EXP-CL09](EXP-CL09-instruction-arm-belief-repair/experiment.md) | **Claude track.** Calibration: K1 rebuilt on the memo's belief construct (Bennett-style scoped probe) + arm-B text repair removing the enforcement implicature ('release condition'), at the memo-preferred, never-run cell (pressure 0, `named_dependent`), arms A/B x observation, n=30 | complete | **Manipulation valid, comparison arm still ceilinged.** K1-belief 240/240 (and the v1 contamination finding retracted as probe artifact); K2 passes at named_dependent for the first time (+26.7pp, p=0.031); K5 fails — arm B 90% observed / 100% unobserved, gap −10pp. The rule-text hypothesis is disconfirmed; prediction 1 remains untestable on sonnet-5 pending the cost-decoupling redesign | ~$5 |
| [EXP-CL10](EXP-CL10-split-budget-trilemma/experiment.md) | **Claude track.** Calibration of the new `claude_benjamin_split_budget` instrument: six candidates / four slots decouple compliance cost from commitment-breaking, testing whether K2, K5, and K6 can hold simultaneously for the first time. Arms A/B/C x observation, `named_dependent`, n=30 | complete | **The trilemma inverted.** K1-belief 180/180; K5 passes for the first time ever (arm B 93/87); K6 passes decisively (88% affirm, 100% keeping); K2 breaks in an unanticipated shape (arm A 33/50, mid-range but observation-insensitive) — DC5 fires, no memo verdict. Unregistered: matched transfer separates C from B for the first time (13/26 vs 2/26 unobserved, p=0.0016), the memo's prediction-2 signature | ~$8 |
| [EXP-CL11](EXP-CL11-cover-story-revision/experiment.md) | **Claude track.** Calibration spending the one allowed revision: bonus items lose their authority framing so defection loses its alibi. Arm A x observation, n=30 — does K2 return? | complete | **K2 fails a second time, same shape — the stop rule fires.** The inverted gap replicates exactly (−16.7pp in both batches; pooled 28/60 obs vs 38/60 unobs, p=0.098 two-sided) while K1-belief is 60/60 (480/480 cumulative). Cover-story hypothesis disconfirmed; new hypothesis recorded: responsibility diffusion — an announced auditor licenses omission of the unticketed repair. No further K2 revisions; EXP-CL12 blocked | ~$2 |
...
Total API spend logged: **$594.27** (plus $0.73 in phase-4 smoke tests and
