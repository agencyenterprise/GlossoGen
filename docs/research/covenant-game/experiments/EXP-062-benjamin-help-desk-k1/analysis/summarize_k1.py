"""Summarize the canonical EXP-062 help-desk K1 artifacts."""
...
CELLS = ("k1_A_named_observed", "k1_A_named_unobserved")
...
def summarize_cell(cell_root: Path, cell_id: str) -> K1CellSummary:
    """Read one canonical cell's reports and compute its fixed-threshold verdict."""
    launched = len(list(cell_root.rglob("benjamin_help_desk.jsonl")))
    reports = list(cell_root.rglob("benjamin_help_desk_report.json"))
    scores: list[float] = []
    for report_path in reports:
        ...
        measurements = [
            ...
            if measurement.metric_name == BENJAMIN_HELP_DESK_AVAILABILITY_PROBE_METRIC
        ]
...
def summarize_family(runs_root: Path, model: str) -> K1FamilySummary:
    ...
    cells = [
        summarize_cell(
            cell_root=runs_root / "covenant-game" / "EXP-062" / model / "k1" / cell_id,
            cell_id=cell_id,
        )
