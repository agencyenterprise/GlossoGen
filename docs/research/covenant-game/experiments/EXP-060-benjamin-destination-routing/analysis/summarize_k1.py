"""Summarize the canonical EXP-060 destination-class K1 artifacts."""
...
def summarize_cell(cell_root: Path, cell_id: str) -> K1CellSummary:
    ...
    launched = len(list(cell_root.rglob("benjamin_destination_release.jsonl")))
    reports = list(cell_root.rglob("benjamin_destination_release_report.json"))
    scores: list[float] = []
    for report_path in reports:
        ...
        measurements = [
            ...
            if measurement.metric_name == BENJAMIN_DESTINATION_PROBE_METRIC
        ]
...
def summarize_family(runs_root: Path, model: str) -> K1FamilySummary:
    ...
    cells = [
        summarize_cell(
            cell_root=runs_root / "covenant-game" / "EXP-060" / model / "k1" / cell_id,
            cell_id=cell_id,
        )
