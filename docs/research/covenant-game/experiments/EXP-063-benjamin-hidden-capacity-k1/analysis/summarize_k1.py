"""Summarize the canonical EXP-063 hidden-capacity K1 artifacts."""
...
def summarize_cell(cell_root: Path, cell_id: str) -> K1CellSummary:
    ...
    launched = len(list(cell_root.rglob("benjamin_capacity_reservation.jsonl")))
    reports = list(cell_root.rglob("benjamin_capacity_reservation_report.json"))
    scores: list[float] = []
...
def summarize_family(runs_root: Path, model: str) -> K1FamilySummary:
    ...
    cells = [
        summarize_cell(
            cell_root=runs_root / "covenant-game" / "EXP-063" / model / "k1" / cell_id,
            cell_id=cell_id,
        )
