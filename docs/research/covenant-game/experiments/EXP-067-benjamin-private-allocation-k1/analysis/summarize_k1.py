"""Summarize the canonical EXP-067 private-allocation K1 artifacts."""
...
def summarize_cell(cell_root: Path, cell_id: str) -> K1CellSummary:
    ...
    launched = len(list(cell_root.rglob("benjamin_private_allocation.jsonl")))
    reports = list(cell_root.rglob("benjamin_private_allocation_report.json"))
    scores: list[float] = []
    for report_path in reports:
        ...
        measurements = [
            ...
            if measurement.metric_name == BENJAMIN_PRIVATE_ALLOCATION_OBSERVABILITY_PROBE
        ]
...
def summarize_family(runs_root: Path, model: str) -> K1FamilySummary:
    ...
    cells = [
        summarize_cell(
            cell_root=runs_root / "covenant-game" / "EXP-067" / model / "k1" / cell,
            cell_id=cell,
        )
        for cell in CELLS
    ]
