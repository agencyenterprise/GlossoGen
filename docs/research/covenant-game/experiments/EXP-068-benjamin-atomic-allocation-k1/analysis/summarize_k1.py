"""Summarize the canonical EXP-068 atomic-allocation K1 artifacts."""
...
def summarize_cell(cell_root: Path, cell_id: str) -> K1CellSummary:
    ...
    launched = len(list(cell_root.rglob("benjamin_atomic_allocation.jsonl")))
    reports = list(cell_root.rglob("benjamin_atomic_allocation_report.json"))
    scores: list[float] = []
...
def summarize_family(runs_root: Path, model: str) -> K1FamilySummary:
    ...
    cells = [
        summarize_cell(
            cell_root=runs_root / "covenant-game" / "EXP-068" / model / "k1" / cell,
            cell_id=cell,
        )
        for cell in CELLS
    ]
