...
**Immediate next steps.** Decompose the covenant treatment into wording versus the act of affirming, then build toward NCRI's proposed direction — the social cost of defection, framed on observation-sensitivity and transfer rather than absolute compliance.
...
| 2026-08-28 | W2 — Decompose the covenant treatment | Wording crossed against the act of affirming: rule text with an affirmation step, covenant text without one. Resolves the one genuine unknown in the W1 result | 🟡 Next |
| TBD | W3 — Social cost of defection ("Benjamin Test") instrument | Instrument where breach carries a social consequence, framed on observation-sensitivity and transfer rather than absolute compliance | ⬜ Planned |
| TBD | W4 — `service_reliability` calibration closed or instrument retired | Two open calibration failures resolved, or the instrument retired with the diagnosis recorded | ⬜ Planned |
