<!--
DRAFT 2 of 2 — for the NCRI × AE Studio shared Google Doc.

Structure deliberately mirrors the approved Schmidt Sciences Phase 1 report:
short body (about / at-a-glance / abstract / one worked concrete
example / how we measure / generalization / how to read on), then
"# Appendix: full details" organised BY INSTRUMENT, each with its own
"Example messages from real runs" section, closing with a
"where this points next" section.

Differences from that template, both deliberate:
  - No platform section and no repo links. The simulation platform is internal
    and is not a deliverable of this engagement, so Appendix B documents the
    *method* (the abstractions a reader needs) rather than promoting a tool.
  - No API cost figures anywhere.

All transcripts in this document are verbatim, pulled from run logs.
No em dashes outside quoted transcript text. Run IDs are
given so any of them can be re-pulled. Do NOT replace them with invented
examples.
