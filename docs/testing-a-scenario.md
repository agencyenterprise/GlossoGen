...
The preset is required for the same reason `--config` is required on `run`:
nothing picks a configuration on your behalf, and the test exercises a
configuration you actually ship. Each agent is routed to a primary channel it
