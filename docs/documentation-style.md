...
- **No machine tells.** The aphorism contrast ("the budget is a knob, not a
  ...
  cloned, it is Y"), semicolons splicing clauses a period would separate (a
  semicolon stays for a list whose items carry commas), inflated words (robust,
...
| Semicolons | ≤ 4 per 1,000 prose words | The clause splice reads generated; the list whose items carry commas does not |

A band triggers a review. A page over one gets read again, and
