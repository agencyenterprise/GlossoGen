...
an eavesdropper on the shared channel, so speaking plainly loses the round.
[Scenarios](scenarios.md) has the per-scenario numbers.
...
tool call and model response lands in a JSONL event log, which is the ledger
everything else reads: evaluation metrics, the web UI, and the flows that rewind
a finished run and replay it with a different agent in one seat.
...
the repository;
[Installation](installation.md#working-on-glossogen-itself) covers it.
...
[Running simulations](running-simulations.md) covers the providers,
...
picks it up, with no change to the glossogen repository.
[As a dependency](installation.md#as-a-dependency) has the `.env` layout, the
optional extras, and where to go from there.
...
[Understanding cost](running-simulations.md#understanding-cost).

[Quickstart](quickstart.md) walks one run end to end.
[Running simulations](running-simulations.md) has knobs and presets, per-agent
...
catalogue and the hook table are in [Evaluation](evaluation.md).
...
Omit it to serve the API alone. See [Web UI](web-ui.md).

## Contributing

[CONTRIBUTING.md](../CONTRIBUTING.md) covers the conventions, the test suite and
how releases are cut. `make lint` and `make test` need to pass before a pull
request. For anything security-related, follow [SECURITY.md](../SECURITY.md)
