...
from glossogen.server.identity.middleware import ClerkIdentityMiddleware
from glossogen.server.identity.settings import load_identity_settings
from glossogen.server.identity.webhook_router import router as clerk_webhook_router
from glossogen.server.mcp.browser import mount_mcp_browser
from glossogen.server.mcp.consent_router import router as mcp_consent_router
...
_runs_dir = Path(os.environ.get("GLOSSOGEN_RUNS_DIR", "./runs"))
...
app = FastAPI(title="GlossoGen Simulation Server", lifespan=lifespan)
...
if _oauth_issuer_url is not None:
    ...
    _oauth_provider = GlossoGenOAuthProvider(
