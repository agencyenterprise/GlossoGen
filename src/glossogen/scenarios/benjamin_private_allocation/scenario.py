"""Private-allocation Benjamin instrument with neutral work-phase prompts."""
...
from glossogen.scenarios.benjamin_private_allocation.evaluation.metric_names import (
    BENJAMIN_PRIVATE_ALLOCATION_METRIC_NAMES,
)
from glossogen.scenarios.benjamin_private_allocation.events import (
    BenjaminPrivateAllocationStatusRouted,
)
...
class BenjaminPrivateAllocationScenario(BenjaminReleasePipelineScenario):
    ...
    def get_available_metric_names(cls) -> list[str]:
        ...
        return sorted({*generic_names, *BENJAMIN_PRIVATE_ALLOCATION_METRIC_NAMES})

    @classmethod
    def knobs_model(cls) -> type[BenjaminPrivateAllocationKnobs]:
        """Return the validated private-allocation configuration model."""
        return BenjaminPrivateAllocationKnobs

    @classmethod
    def create_from_config(cls, config: dict[str, Any]) -> Self:
        """Reconstruct the scenario from persisted configuration."""
        return cls(knobs=BenjaminPrivateAllocationKnobs.model_validate(config))

    def __init__(self, knobs: BenjaminPrivateAllocationKnobs) -> None:
        self._knobs = knobs
        self._renderer = TemplateRenderer(prompts_dirs=[PROMPTS_DIR])
        self._world = BenjaminPrivateAllocationWorld(knobs=knobs)

    def name(self) -> str:
        """Return the stable scenario identifier."""
        return "benjamin_private_allocation"
    ...
    def get_mcp_tools(self) -> list[ScenarioMcpTool]:
        """Return the private-allocation structured actions."""
    ...
    async def on_round_ended(self, round_number: int, trigger: str) -> None:
        ...
        await self.runtime.event_logger.log(
            event=BenjaminPrivateAllocationStatusRouted(
