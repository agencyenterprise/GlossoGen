...
RESPONSES_FILE_NAME = "benjamin_private_allocation_probe_response.json"
USAGE_FILE_NAME = "benjamin_private_allocation_probe_usage.json"
...
class BenjaminPrivateAllocationObservabilityProbeMetric(Metric):
    """Probe whether the focal status is represented as externally observable."""

    name = BENJAMIN_PRIVATE_ALLOCATION_OBSERVABILITY_PROBE
    ...
    async def compute(
        ...
        if scenario.name() != "benjamin_private_allocation":
            return []
        knobs = BenjaminPrivateAllocationKnobs.model_validate(scenario.get_scenario_config())
