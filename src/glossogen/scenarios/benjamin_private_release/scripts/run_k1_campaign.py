"""Launch a frozen K1 campaign with frontend links published while runs are active."""
...
SCENARIO_NAME = "benjamin_private_release"


class K1CampaignManifest(BaseModel):
    """Immutable smoke and K1 matrix for one private-release calibration."""
    ...
    def validate_matrix(self) -> Self:
        ...
        if set(self.stages) != {"smoke", "k1"}:
            raise ValueError("K1 calibration manifest must contain smoke and k1 stages")
...
def validate_k1_evaluation_artifact(run_dir: Path) -> RunArtifactValidation:
    """Require both source-aligned probe sidecars after K1 evaluation."""
    missing = [
        file_name
...
async def validate_run_artifact(run_dir: Path) -> RunArtifactValidation:
    """Validate one private-release run from its typed JSONL events."""
...
def _evaluation_command(run_dir: Path, model: str, provider: str) -> list[str]:
    """Build the held-out source-aligned K1 evaluation command."""
    return [
        sys.executable,
        "-m",
        "glossogen",
...
async def _run_job(
    ...
    try:
        ...
        logger_message = f"[{job.ordinal:03d}] {exc}"
