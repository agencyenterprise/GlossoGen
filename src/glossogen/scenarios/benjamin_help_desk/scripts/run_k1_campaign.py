"""Launch the frozen help-desk K1 campaign with live frontend publication."""
...
from glossogen.scenarios.benjamin_help_desk.evaluation.metric_names import (
    BENJAMIN_HELP_DESK_AVAILABILITY_PROBE_METRIC,
)
from glossogen.scenarios.benjamin_help_desk.events import BenjaminHelpDeskShiftCompleted
...
SCENARIO_NAME = "benjamin_help_desk"
logger = logging.getLogger(__name__)


class K1CampaignManifest(BaseModel):
    """Immutable smoke and K1 matrix for the help-desk calibration."""
    ...
    def validate_matrix(self) -> Self:
        """Require a complete two-cell calibration over three seeds."""
...
def validate_k1_evaluation_artifact(run_dir: Path) -> RunArtifactValidation:
    """Require both structured-probe sidecars after K1 evaluation."""
    missing = [
        file_name
...
async def validate_run_artifact(run_dir: Path) -> RunArtifactValidation:
    ...
    endpoints = [event for event in events if isinstance(event, BenjaminHelpDeskShiftCompleted)]
    if len(endpoints) != 1:
        return RunArtifactValidation(
            valid=False,
            reason=f"expected one help-desk endpoint, found {len(endpoints)}",
        )
    if not endpoints[0].completed_by_agent:
        return RunArtifactValidation(
            valid=False,
            reason="shift endpoint was frozen by timeout rather than the agent",
        )
...
async def run_campaign_stage(
    ...
    """Consume jobs and stop dispatch after the first operational failure."""
