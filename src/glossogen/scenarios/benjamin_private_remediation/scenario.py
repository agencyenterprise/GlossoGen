"""Private-remediation Benjamin instrument with neutral work-phase prompts."""
...
from glossogen.scenarios.benjamin_private_remediation.evaluation.metric_names import (
    BENJAMIN_PRIVATE_REMEDIATION_METRIC_NAMES,
)
from glossogen.scenarios.benjamin_private_remediation.events import (
    BenjaminPrivateRemediationStatusRouted,
)
from glossogen.scenarios.benjamin_private_remediation.knobs import (
    BenjaminPrivateRemediationKnobs,
    ObservationMode,
)
from glossogen.scenarios.benjamin_private_remediation.mcp_tools import (
    ACKNOWLEDGE_SETUP_TOOL,
    INSPECT_CANDIDATES_TOOL,
    RECORD_LOCAL_NOTE_TOOL,
    SCHEDULE_CONTINUITY_TOOL,
...
class BenjaminPrivateRemediationScenario(BenjaminReleasePipelineScenario):
    ...
    def get_available_metric_names(cls) -> list[str]:
        ...
        return sorted({*generic_names, *BENJAMIN_PRIVATE_REMEDIATION_METRIC_NAMES})

    @classmethod
    def knobs_model(cls) -> type[BenjaminPrivateRemediationKnobs]:
        """Return the validated private-remediation configuration model."""
        return BenjaminPrivateRemediationKnobs

    @classmethod
    def create_from_config(cls, config: dict[str, Any]) -> Self:
        """Reconstruct the scenario from persisted configuration."""
        return cls(knobs=BenjaminPrivateRemediationKnobs.model_validate(config))

    def __init__(self, knobs: BenjaminPrivateRemediationKnobs) -> None:
        self._knobs = knobs
        self._renderer = TemplateRenderer(prompts_dirs=[PROMPTS_DIR])
        self._world = BenjaminPrivateRemediationWorld(knobs=knobs)

    def name(self) -> str:
        """Return the stable scenario identifier."""
        return "benjamin_private_remediation"
    ...
    def get_mcp_tools(self) -> list[ScenarioMcpTool]:
        """Return the private-remediation structured actions."""
    ...
    async def on_round_ended(self, round_number: int, trigger: str) -> None:
        ...
        await self.runtime.event_logger.log(
            event=BenjaminPrivateRemediationStatusRouted(
