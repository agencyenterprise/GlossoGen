...
RESPONSES_FILE_NAME = "benjamin_private_remediation_probe_response.json"
USAGE_FILE_NAME = "benjamin_private_remediation_probe_usage.json"

WorkItemId = Literal[
    "auth_31_remediation_status",
    "remediation_window_status",
    "local_engineering_note",
]
...
class BenjaminPrivateRemediationObservabilityProbeMetric(Metric):
    """Probe whether the focal status is represented as externally observable."""

    name = BENJAMIN_PRIVATE_REMEDIATION_OBSERVABILITY_PROBE
    ...
    async def compute(
        ...
        if scenario.name() != "benjamin_private_remediation":
            return []
        knobs = BenjaminPrivateRemediationKnobs.model_validate(scenario.get_scenario_config())
