"""Launch the frozen capacity-reservation K1 campaign with live frontend publication."""
...
from glossogen.scenarios.benjamin_capacity_reservation.events import BenjaminCapacityShiftCompleted
...
SCENARIO_NAME = "benjamin_capacity_reservation"
logger = logging.getLogger(__name__)


class K1CampaignManifest(BaseModel):
    """Immutable smoke and K1 matrix for capacity-reservation calibration."""
    ...
    def validate_matrix(self) -> Self:
        """Require a complete two-cell calibration over three fresh seeds."""
            ...
            for cell_id in stage.cell_order:
                for seed in required_seeds:
                    if (stage_name, cell_id, seed) not in config_keys:
                        raise ValueError(
                            f"missing config for stage={stage_name}, "
...
def validate_k1_evaluation_artifact(run_dir: Path) -> RunArtifactValidation:
    """Require both structured-probe sidecars and one K1 measurement."""
    missing = [
        file_name
    ...
    measurements = [
        ...
        if measurement.metric_name == BENJAMIN_CAPACITY_AVAILABILITY_PROBE_METRIC
    ]
...
async def validate_run_artifact(run_dir: Path) -> RunArtifactValidation:
    """Require normal completion and one agent-frozen mechanical endpoint."""
    ...
    endpoints = [event for event in events if isinstance(event, BenjaminCapacityShiftCompleted)]
    if len(endpoints) != 1:
        return RunArtifactValidation(
            valid=False,
            reason=f"expected one capacity endpoint, found {len(endpoints)}",
        )
    if not endpoints[0].completed_by_agent:
        return RunArtifactValidation(
            valid=False,
            reason="capacity endpoint was frozen by timeout rather than the agent",
        )
    if endpoints[0].partner_capacity_reserved is None:
        return RunArtifactValidation(
            valid=False,
            reason="agent-completed trajectory has no binary capacity allocation",
        )
...
def publish_frontend_link(
    ...
    """Expose a nested run to the frontend before trajectory completion."""
...
def _simulation_command(
    ...
    """Build the secret-free simulation command."""
    return [
        sys.executable,
        "-m",
        "glossogen",
...
async def _discover_and_publish_live_link(
    ...
    """Publish the frontend link as soon as the run directory appears."""
...
async def _run_job(
    ...
    try:
        ...
        logger.exception("Live frontend publication failed")
        await process.wait()
        raise
...
async def run_campaign_stage(
    ...
    """Consume jobs and stop dispatch after an operational or frozen K1 failure."""
