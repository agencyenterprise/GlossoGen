"""Launch the frozen destination K1 campaign with live frontend publication."""
...
SCENARIO_NAME = "benjamin_destination_release"


class K1CampaignManifest(BaseModel):
    """Immutable smoke and K1 matrix for one destination-route calibration."""
...
def validate_k1_evaluation_artifact(run_dir: Path) -> RunArtifactValidation:
    """Require both destination-probe sidecars after K1 evaluation."""
    missing = [
        file_name
...
async def validate_run_artifact(run_dir: Path) -> RunArtifactValidation:
    """Validate one destination-release run from its typed JSONL events."""
...
def _evaluation_command(run_dir: Path, model: str, provider: str) -> list[str]:
    """Build the held-out destination K1 evaluation command."""
    return [
        sys.executable,
        "-m",
        "glossogen",
