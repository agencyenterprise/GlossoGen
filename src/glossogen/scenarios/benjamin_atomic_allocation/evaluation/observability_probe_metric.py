...
RESPONSES_FILE_NAME = "benjamin_atomic_allocation_probe_response.json"
USAGE_FILE_NAME = "benjamin_atomic_allocation_probe_usage.json"
...
class BenjaminAtomicAllocationObservabilityProbeMetric(Metric):
    """Probe whether the focal status is represented as externally observable."""

    name = BENJAMIN_ATOMIC_ALLOCATION_OBSERVABILITY_PROBE
    ...
    async def compute(
        ...
        if scenario.name() != "benjamin_atomic_allocation":
            return []
        knobs = BenjaminAtomicAllocationKnobs.model_validate(scenario.get_scenario_config())
