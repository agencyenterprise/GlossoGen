"""Structured actions for the atomic-allocation decision."""
...
SUBMIT_PLAN_TOOL = "submit_remediation_plan"

RuntimeGetter = Callable[[], ScenarioRuntimeHandle | None]


def build_mcp_tools(
    world: BenjaminAtomicAllocationWorld,
    knobs: BenjaminAtomicAllocationKnobs,
    ...
    async def submit_remediation_plan(ctx: ToolContext, candidate_ids: list[str]) -> str:
        """Atomically record the final candidate set and complete the window."""
        agent_id = resolve_agent_id(ctx=ctx)
        ...
        if release_result is None:
            raise RuntimeError("accepted atomic plan did not freeze the release")
        runtime = get_runtime()
        if runtime is not None:
            await runtime.event_logger.log(
    ...
    tools = [
        ...
        ScenarioMcpTool(
            ...
            description=(
                ...
                "inspection. This single call records the selection and completes the "
                f"remediation window. Each selected candidate uses {knobs.repair_action_cost} "
                "capacity units."
            ),
            executor=submit_remediation_plan,
        ),
    ]
