...
from glossogen.scenarios.benjamin_atomic_allocation.evaluation.metric_names import (
    BENJAMIN_ATOMIC_ALLOCATION_METRIC_NAMES,
)
from glossogen.scenarios.benjamin_atomic_allocation.events import (
    BenjaminAtomicAllocationStatusRouted,
)
...
class BenjaminAtomicAllocationScenario(BenjaminReleasePipelineScenario):
    ...
    def get_available_metric_names(cls) -> list[str]:
        ...
        return sorted({*generic_names, *BENJAMIN_ATOMIC_ALLOCATION_METRIC_NAMES})

    @classmethod
    def knobs_model(cls) -> type[BenjaminAtomicAllocationKnobs]:
        """Return the validated atomic-allocation configuration model."""
        return BenjaminAtomicAllocationKnobs

    @classmethod
    def create_from_config(cls, config: dict[str, Any]) -> Self:
        """Reconstruct the scenario from persisted configuration."""
        return cls(knobs=BenjaminAtomicAllocationKnobs.model_validate(config))

    def __init__(self, knobs: BenjaminAtomicAllocationKnobs) -> None:
        self._knobs = knobs
        self._renderer = TemplateRenderer(prompts_dirs=[PROMPTS_DIR])
        self._world = BenjaminAtomicAllocationWorld(knobs=knobs)

    def name(self) -> str:
        """Return the stable scenario identifier."""
        return "benjamin_atomic_allocation"
    ...
    def get_mcp_tools(self) -> list[ScenarioMcpTool]:
        """Return the atomic-allocation structured actions."""
    ...
    async def on_round_ended(self, round_number: int, trigger: str) -> None:
        ...
        await self.runtime.event_logger.log(
            event=BenjaminAtomicAllocationStatusRouted(
