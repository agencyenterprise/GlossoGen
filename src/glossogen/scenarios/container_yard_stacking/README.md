...
```bash
VIRTUAL_ENV= uv run --no-sync python -m glossogen run container_yard_stacking \
  --model claude-sonnet-4-6 --provider anthropic --runs-dir ./runs \
  --config src/glossogen/scenarios/container_yard_stacking/knobs_default.json \
...
```bash
VIRTUAL_ENV= uv run --no-sync python -m glossogen evaluate container_yard_stacking \
