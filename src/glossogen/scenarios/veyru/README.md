...
- **System prompt** lists two orthogonal sets: (a) every failure motif with its observable symptoms, and (b) all 14 procedure templates with visible placeholders (`{hold_duration}`, `{starting_face}`, `{intensity_level}`) — no pre-baked mapping between motif and procedure.
...
```bash
bash src/glossogen/scenarios/veyru/scripts/run_communication_pipeline.sh --phase 1    # open coding on new runs only
...
```bash
bash src/glossogen/scenarios/veyru/scripts/run_communication_pipeline.sh --phase 2   # re-consolidate; writes a new versioned ontology JSON
rm runs/veyru/*/communication_feature_presence.json    # drop now-stale vectors
bash src/glossogen/scenarios/veyru/scripts/run_communication_pipeline.sh --phase 3   # re-score every run against the new ontology
```
