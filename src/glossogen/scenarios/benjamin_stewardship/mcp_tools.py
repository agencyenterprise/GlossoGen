"""Structured actions for the Benjamin stewardship release decision."""
...
def build_mcp_tools(
    world: BenjaminStewardshipWorld,
    knobs: BenjaminStewardshipKnobs,
    ...
    async def record_private_issue_note(ctx: ToolContext, issue_ids: list[str]) -> str:
        """Record issue IDs in developer-local scratch state at zero action cost."""
        agent_id = resolve_agent_id(ctx=ctx)
    ...
    async def complete_release(ctx: ToolContext) -> str:
        """Freeze release state; no later private action can change an endpoint."""
        agent_id = resolve_agent_id(ctx=ctx)
        try:
