...
from glossogen.scenarios.benjamin_stewardship.evaluation.metric_names import BENJAMIN_METRIC_NAMES
from glossogen.scenarios.benjamin_stewardship.events import (
...
from glossogen.scenarios.benjamin_stewardship.knobs import BenjaminArm, BenjaminStewardshipKnobs
from glossogen.scenarios.benjamin_stewardship.mcp_tools import build_mcp_tools
from glossogen.scenarios.benjamin_stewardship.world import BenjaminStewardshipWorld
...
class BenjaminStewardshipScenario(SimulationScenario):
    ...
    @classmethod
    def knobs_model(cls) -> type[BenjaminStewardshipKnobs]:
        """Return the validated configuration model."""
        return BenjaminStewardshipKnobs
    ...
    def create_from_config(cls, config: dict[str, Any]) -> Self:
        ...
        return cls(knobs=BenjaminStewardshipKnobs.model_validate(config))

    def __init__(self, knobs: BenjaminStewardshipKnobs) -> None:
        self._knobs = knobs
        self._renderer = TemplateRenderer(prompts_dirs=[PROMPTS_DIR])
        self._world = BenjaminStewardshipWorld(knobs=knobs)

    def name(self) -> str:
        """Return the stable scenario identifier."""
        return "benjamin_stewardship"

    def get_knobs(self) -> BenjaminStewardshipKnobs:
        """Return the active validated configuration."""
        return self._knobs

    def get_world(self) -> ScenarioWorld:
        """Return the deterministic private-workspace state machine."""
        return self._world
    ...
    def get_channels(self) -> list[Channel]:
        """Return the setup record that closes before private work."""
        return [
            Channel(
    ...
    def get_channel_display_name(self, channel_id: str, agent_id: str) -> str:
        ...
        if channel_id != SETUP_CHANNEL_ID:
            raise ValueError(f"unknown Benjamin stewardship channel: {channel_id}")
        return SETUP_CHANNEL_NAME

    def get_agent_display_name(self, agent_id: str) -> str:
        """Return the display name for the sole live role."""
        if agent_id != DEVELOPER_ID:
            raise ValueError(f"unknown Benjamin stewardship agent: {agent_id}")
    ...
    def validate_outgoing_message(self, agent_id: str, channel_id: str) -> str | None:
        """Reject messages; private state has no communication route."""
        _ = agent_id, channel_id
        return "the release setup record accepts no developer-authored messages"

    def get_mcp_tools(self) -> list[ScenarioMcpTool]:
        """Return the active setup and private-workspace actions."""
    ...
    async def on_round_advanced(self, round_number: int) -> None:
        ...
        if round_number == 2:
            await self.runtime.update_channel_members(
                channel_id=SETUP_CHANNEL_ID,
                member_agent_ids=[],
                reason="private deployment work began",
            )

    def get_early_round_end_trigger(self) -> str | None:
        """End setup on its decision and private work on release completion."""
