...
instrumentation once per ``schmidt run`` process. Every simulation agent's LLM
...
the run path keeps the separate ``schmidt evaluate`` process untraced.
...
from schmidt.telemetry_round_processor import RoundStampingSpanProcessor, current_round_source
from schmidt.telemetry_settings import TelemetrySettings
