...
class ScenarioRunDetailExtension(ABC):
    ...
    Subclasses live in ``schmidt.scenarios.<name>.run_detail_extension``.
...
def discover_scenario_extensions() -> dict[str, ScenarioRunDetailExtension]:
    ...
    Walks :mod:`schmidt.scenarios`, imports each
    ``<scenario_pkg>.run_detail_extension`` submodule when present, and
    ...
    for module_info in pkgutil.iter_modules(schmidt.scenarios.__path__):
        if not module_info.ispkg:
            continue
        module_name = f"schmidt.scenarios.{module_info.name}.run_detail_extension"
        try:
