...
```bash
VIRTUAL_ENV= uv run --no-sync python -m schmidt run satellite_contact_window \
  --model claude-sonnet-4-6 --provider anthropic --runs-dir ./runs \
  --config src/schmidt/scenarios/satellite_contact_window/knobs_default.json \
  > ./runs/satellite_stdout.log 2>&1 &

VIRTUAL_ENV= uv run --no-sync python -m schmidt evaluate satellite_contact_window \
