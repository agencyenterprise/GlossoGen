...
from schmidt.scenarios.spillway_release.agent_factory import (
    build_agent_display_names,
    build_agents,
    build_channel_display_names,
    build_channels,
)
from schmidt.scenarios.spillway_release.case_event_conversion import case_started_event
from schmidt.scenarios.spillway_release.events import SpillwayRoundResolved
...
from schmidt.scenarios.spillway_release.injection_rendering import (
...
from schmidt.scenarios.spillway_release.mcp_tools import build_mcp_tools
from schmidt.scenarios.spillway_release.spillway_cases import get_cases
from schmidt.scenarios.spillway_release.world import SpillwayWorld
...
class SpillwayReleaseScenario(SimulationScenario):
    ...
    def get_scenario_config(self) -> dict[str, object]:
        """Return spillway knobs as a config dict for the JSONL log."""
    ...
    def bind_runtime(self, runtime: ScenarioRuntimeHandle) -> None:
        """Stash the runtime handle so the action tools can emit verdict events."""
    ...
    def _get_metrics(self) -> dict[str, type[Metric]]:
        """Return spillway-specific metric classes keyed by metric name (none)."""
        return {}
