...
set -a && source .env && set +a && \
  VIRTUAL_ENV= uv run --no-sync python -m schmidt run product_launch_v2 \
  --model <model> --runs-dir ./runs \
  --knobs src/schmidt/scenarios/product_launch_v2/knobs_baseline.json \
  > ./runs/product_launch_v2_stdout.log 2>&1 &
```
