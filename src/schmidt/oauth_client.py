"""OAuth 2.0 PKCE client for ``schmidt login`` against a remote MCP server.
...
then persists the resulting tokens to ``~/.schmidt/credentials.json``.
...
def load_credentials(*, path: Path) -> Credentials:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"No credentials at {path}; run `schmidt login` first to authenticate."
        )
...
async def refresh_credentials(*, credentials: Credentials) -> Credentials:
    ...
    rejects the refresh — callers should fall back to ``schmidt login``.
    """
    if credentials.refresh_token is None:
        raise RuntimeError("No refresh_token in credentials; run `schmidt login` again.")
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            raise RuntimeError(
                f"Refresh failed ({exc.response.status_code}); run `schmidt login` again."
            ) from exc
...
def delete_credentials(*, path: Path) -> None:
    """Remove the credentials file (used by ``schmidt logout``-style flows)."""
    if path.exists():
        path.unlink()
