...
The manifest's anchor describes a pristine clone, whose last event is that anchor. A fork that crashed after playing past its boundary has appended events, so ``load_resume_state`` resumes it from its own last message like a plain ``--resume`` (keeping the manifest's agent filters), instead of re-anchoring at the boundary — which would discard the fork's progress and, on a final-round fork, log a second ``RoundAdvanced(trigger="fork_after_round")``. A progressed cross-run fork is refused instead: the imported agent's history is rebuilt exclusively from source B's events, so its post-boundary turns cannot be re-seeded.
